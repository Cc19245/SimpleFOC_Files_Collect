...
//添加命令，主要是为了注册命令提示符、注册命令回调函数
//分别是id-命令的提示符， onCommand-命令的回调函数，  label-命令的提示标签，不起实际作用
...
void Commander::run(){
  ...
  while (com_port->available()) {     //只要有串口数据
    // get the new byte:
    received_chars[rec_cnt] = (char)com_port->read(); //一直读取
    // end of user input
    if (received_chars[rec_cnt++] == '\n') {  //上位机串口最后发来一个换行符\n
      ...
      received_chars[0] = 0;  //私有成员变量，接收到的数据的缓冲区
      rec_cnt=0;   //私有成员变量，接收到的数据的个数
    }
  }
}
...
//这才是针对不同的串口命令进行不同的操作的函数
void Commander::run(char* user_input){  
  // execute the user command
  char id = user_input[0];   //命令分类
  switch(id){    //前三种都是对命令和命令回传的格式进行设置，也就是通信本身的设置
    case CMD_SCAN:   
    ...
    case CMD_DECIMAL:
      ...
      break;
    default:   //这才是真正的操作数据的设置
      for(int i=0; i < call_count; i++){  //在所有的已注册的command里寻找匹配的命令
        if(id == call_ids[i]){   //命令首字符匹配
          call_list[i](&user_input[1]);   //调用回调函数，传入命令随带的数据
          break;
        }
      }
      break;
...
//Arduino中改变目标值的函数，也就是void doTarget(char* cmd)的回调函数，实际内部执行的就是这个函数。
//参数：value-要改变的变量，  user_cmd-用户输入的变量值
void Commander::scalar(float* value,  char* user_cmd){
  bool GET  = user_cmd[0] == '\n';
  if(!GET) *value = atof(user_cmd);  //atof:Arduino库函数，将字符串转换为双精度浮点型值。
  println(*value);  //串口打印这个变量值
}
